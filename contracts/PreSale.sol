...
contract ICOMarketPlace {
     struct TokenDetails {
        address token;
        bool supported; //when the user add token it will turn into true
        uint256 price;
        address creator;
        string name;
        string symbol;
    ...
    modifier onlyCreator(address _token){
        require(msg.sender==tokenDetails[_token].creator, "caller is not creator");
        _;
    }

    receive() external payable{
        revert("cannot take ethers directly");
    }
    ...
    //fun multiply used to overcome the error of underflow and overflow
    ...
    function buyToken(address _token, uint256 _amount)external payable supportedToken(_token){
        ...
        //TRANSFERING THE TOKEN TO THE BUYER
        IERC20 token = IERC20(_token);
    ...
    function getTokenCreatedBy(address _creator)external view returns(TokenDetails[] memory){
        ...
        uint256 index = 0;
         if(tokenDetails[allSupportedTokens[index]].creator == _creator){
            tokens[index] = tokenDetails[allSupportedTokens[index]];
            index++;
         }
